...
const SidebarMenu = ({ menu, isOpen }: SidebarMenuProps) => {
    ...
    const [renderMenuItemVisible, setRenderMenuItemVisible] = useState<
        boolean[]
    >(Array(menu.submenu?.length).fill(false));
        ...
        <li
            ...
            className={`mt-4 hover:opacity-100 px-3 py-1 ${
                item.link === currentPath
                    ? 'opacity-100 bg-gray-800 rounded-md '
                    : 'opacity-80'
            }`}
        ...
        <div className="px-6 select-none">
            <div
                ...
                className={`flex items-center w-full my-3 gap-x-3 hover:opacity-100  ${
                    menu.link === currentPath ? 'opacity-100' : 'opacity-80'
                }`}
            >
                {menu.icon}
                {isOpen && menu.title && (
                    <Link
                        href={menu.link || ''}
                        className={`w-full ${
                            menu.link ? 'cursor-pointer' : 'pointer-events-none'
                        }`}
                    >
                        <h1
